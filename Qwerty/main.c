...
__NO_RETURN static void TempTask(void *arg)
{
  ...
  {
    osDelay(1000);
    temperaturte.updateTemp();
    temperaturte.updateHum();
  }
}
...
{
  (void)arg;
  constexpr int MaxMessageSize = 21;
  ...
  for (;;) {
    ...
    temperature = temp.getHum();
    core_util_critical_section_exit();

    ConvertTemperatureToString(temperature, msg, MaxMessageSize);
    _parametr.Send(msg, MaxMessageSize);
  }
}
...
int main(void) {
  ...
  osThreadNew(UsartTask, &temperature, NULL);
  osKernelStart();
